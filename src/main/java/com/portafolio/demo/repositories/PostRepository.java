...
public class PostRepository {
    ...
    private MongoTemplate mongoTemplate;
     ...
     public UpdateResult addComment(String idPost, Coments commentAdded){
         return mongoTemplate.updateFirst(new Query().addCriteria(Criteria.where("_id").is(idPost)),
                 new Update().addToSet("allComments",commentAdded),Post.class);
...
//        return mongoTemplate.find(query,Post.class);
         ...
         return mongoTemplate.aggregate(Aggregation.newAggregation(
                 Aggregation.match(new Criteria().orOperator(
                         ...
                         Criteria.where("description").regex(search,"i"),
                         Criteria.where("name").regex(search,"i"),
                         Criteria.where("technologies").regex(search,"i")
